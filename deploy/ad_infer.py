...
class Predictor:
    def __init__(self) -> None:
        ...
        parser = argsparser()
        FLAGS = parser.parse_args("--model_dir=infer_models/ad --image_file=test.png --device=GPU".split())
        pred_config = PredictConfig(FLAGS.model_dir)
        ...
        self.FLAGS = FLAGS
    def work(self, img: str = 'screen.jpg'):
        #tm=time.time()
        threshold = self.FLAGS.threshold
        res = self.detector.predict([img], threshold)
        if res['boxes_num'] == 0:
            return []
        res = res['boxes']
        end = len(res)
        for idx, box in enumerate(res):
            if box[1] < threshold:
                end = idx
                break
        #print(time.time()-tm, res[:end])
        return res[:end]